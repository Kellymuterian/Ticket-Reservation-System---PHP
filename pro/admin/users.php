...
if (isset($_POST['submit'])) {
    ...
    if (!isset($route_id, $event_id, $first_fee, $second_fee, $date, $time)) {
        ...
        $conn = connect();
        $ins = $conn->prepare("INSERT INTO `ticket`(`event_id`, `route_id`, `date`, `time`, `first_fee`, `second_fee`) VALUES (?,?,?,?,?,?)");
...
if (isset($_POST['edit'])) {
    ...
    if (!isset($route_id, $event_id, $first_fee, $second_fee, $date, $time)) {
        ...
        $conn = connect();
        $ins = $conn->prepare("UPDATE `ticket` SET `event_id`=?,`route_id`=?,`date`=?,`time`=?,`first_fee`=?,`second_fee`=? WHERE id = ?");
...
if (isset($_POST['del_event'])) {
    $con = connect();
    $conn = $con->query("DELETE FROM ticket WHERE id = '" . $_POST['del_event'] . "'");
    if ($con->affected_rows < 1) {
        alert("ticket Could Not Be Deleted. This Route Has Been Tied To Another Data!");
        load($_SERVER['PHP_SELF'] . "$me");
    } else {
